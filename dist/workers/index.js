...
const buildMessageBase = (job) => job.silent
    ? {
...
const pushToUser = async (app, job) => {
    if (!app.firebaseMessaging || !job.userId)
    ...
    for (const batch of chunk(devices, MULTICAST_CHUNK)) {
        ...
        const stale = response.responses
            .map((result, index) => result.error && STALE_TOKEN_CODES.has(result.error.code) ? batch[index].token : null)
 ...
 */
const recordOutcome = async (app, pushLogId, outcome, error) => {
    if (!pushLogId)
        return;
    const status = error
        ? "FAILED"
...
const startWorkers = (app) => {
    const notificationWorker = (0, queue_js_1.createWorker)(notification_queue_js_1.NOTIFICATIONS_QUEUE, async (job) => {
        ...
        if (payload.audience === "all") {
            ...
            if (app.firebaseMessaging) {
                await app.firebaseMessaging.send({ topic: notification_queue_js_1.BROADCAST_TOPIC, ...buildMessageBase(payload) });
            }
    ...
    const analyticsWorker = (0, queue_js_1.createWorker)(analytics_queue_js_1.ANALYTICS_QUEUE, async (job) => {
        const data = job.data;
    ...
    const auditWorker = (0, queue_js_1.createWorker)(constants_js_1.QUEUES.AUDIT, async (job) => {
        const data = job.data;
